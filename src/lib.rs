...
pub fn verify(
    ...
    let enclave_certificate = (match enclave_certificate {
        Value::Bytes(b) => Ok(b),
        _ => Err("enclave certificate decode failure"),
    })?;
    ...
    let user_data = (match user_data {
        Value::Bytes(b) => Ok(b),
        _ => Err("user data decode failure"),
    })?;
    ...
    let public_key = (match public_key {
        Value::Bytes(b) => Ok(b),
        _ => Err("public key decode failure"),
    })?;
